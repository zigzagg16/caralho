...
class Caralho: UITableView {
    ...
    var caralhoDelegate: CaralhoDelegate?
...
extension Caralho: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell:CaralhoCell = tableView.dequeueReusableCell(withIdentifier: cellIdentifier, for: indexPath) as! CaralhoCell
        ...
        if let bg = crl.bg {
            ...
            formattedTitle.addAttribute(NSForegroundColorAttributeName, value: UIColor.init(patternImage: bg), range: NSMakeRange(0, crl.text.count))
